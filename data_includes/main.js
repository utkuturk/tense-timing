...
PreloadZip("https://raw.githubusercontent.com/utkuturk/tense-timing/morphosyntax/chunk_includes/elevenlabs_audio.zip");
PreloadZip("https://raw.githubusercontent.com/utkuturk/tense-timing/morphosyntax/chunk_includes/pictures.zip");
...
const LAMBDA_URL = "https://u7dzjb1y1m.execute-api.us-east-2.amazonaws.com/default/pcibex-s3-recorder";
InitiateRecorder(
  LAMBDA_URL,
...
const gerundForm = v => GERUND_FORMS[v] || (v.endsWith("e") ? `${v.slice(0, -1)}ing` : `${v}ing`);
const sideLabelLower = side => String(side || "").toLowerCase() === "past" ? "past" : "future";
...
const eventPhraseFor = (verb) => `${gerundForm(verb)} ${objectFor(verb)}`.trim();
...
function makeBlockItems(blockVerbs, pastVerbs, entityRotation = 0) {
  ...
  return [
    ...pastVerbs.map(v => ({
      ...
      side: "PAST",
      target_label_sentence: conceptualLabelSentence(entityByVerb[v], v, "PAST"),
    ...
    ...futureVerbs.map(v => ({
      ...
      pic: pictureFor(v, entityByVerb[v]),
      side: "FUTURE",
      target_label_sentence: conceptualLabelSentence(entityByVerb[v], v, "FUTURE"),
      target_canonical_sentence: canonicalSentence(entityByVerb[v], v, "FUTURE")
    }))
  ];
}
...
function buildBlockSequence(blockOrder, withIntro) {
  ...
  blockOrder.forEach((b, index) => {
    ...
    patternOrder.forEach(patternIndex => {
      ...
      const productionItems = orderItemsByTensePattern(b.items, patternIndex, previousEntity);
...
const PRACTICE_ITEMS = [
  {
    ...
    side: "PAST",
    target_label_sentence: conceptualLabelSentence(PRACTICE_ENTITY, "spin", "PAST"),
    target_canonical_sentence: canonicalSentence(PRACTICE_ENTITY, "spin", "PAST")
  },
  {
    verb: "drag",
    ...
    side: "FUTURE",
    target_label_sentence: conceptualLabelSentence(PRACTICE_ENTITY, "drag", "FUTURE"),
    target_canonical_sentence: canonicalSentence(PRACTICE_ENTITY, "drag", "FUTURE")
  }
];
...
newTrial(
  ...
  newText(
    ...
    .center()
    .print(),
  newVoiceRecorder("recording_test_recorder")
    .log()
    .center()
    .print(),
  newButton("recording_test_continue", "Continue")
    ...
    .wait(
      getVoiceRecorder("recording_test_recorder")
        .test.recorded()
        .failure(
          newText("recording_test_missing", "Please make a test recording before continuing.")
        ...
        .and(
          getVoiceRecorder("recording_test_recorder")
            .test.hasPlayed()
            .failure(
              newText("recording_test_playback", "Please play back your recording once before continuing.")
                .css({ "color": "red", "margin-top": "10px" })
                .center()
                .print()
            )
