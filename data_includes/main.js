...
const LAMBDA_URL = "https://u7dzjb1y1m.execute-api.us-east-2.amazonaws.com/default/pcibex-s3-recorder-phon";
InitiateRecorder(
  LAMBDA_URL,
...
const regularityFor = (verb) => IRREGULAR_VERBS.has(verb) ? "IRREGULAR" : "REGULAR";
...
function makeBlockItems(blockVerbs, entityRotation = 0) {
  const irregularVerbs = blockVerbs.filter(v => regularityFor(v) === "IRREGULAR");
  ...
  if (irregularSorted.length !== ENTITIES.length || regularSorted.length !== ENTITIES.length) {
    throw new Error("Each block must contain exactly 3 irregular and 3 regular verbs.");
  }
...
function buildBlockSequence(blockOrder, withIntro) {
  ...
  blockOrder.forEach((b, index) => {
    ...
    patternOrder.forEach(patternIndex => {
      ...
      const productionItems = orderItemsByRegularityPattern(b.items, patternIndex, previousEntity);
