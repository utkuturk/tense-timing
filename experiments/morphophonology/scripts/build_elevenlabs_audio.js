...
function zipOutputFolder() {
  ...
  const result = spawnSync("zip", ["-q", "-r", ZIP_PATH, "."], { cwd: OUT_DIR, stdio: "inherit" });
  if (result.status !== 0) {
...
async function main() {
  ...
  if (!ELEVENLABS_API_KEY || ELEVENLABS_API_KEY === "XX") {
    console.log("ELEVENLABS_API_KEY is not set. Generated manifest/texts only.");
  ...
  for (const entry of entries) {
    ...
    if (!FORCE_REGEN && fs.existsSync(outFile)) continue;
